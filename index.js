...
client.once('ready', () => {
  ...
  // console logs every five minutes to keep it alive
  const pingerInterval = setInterval(pinger, 60 * 5 * 1000);
  function pinger() {
...
client.on('messageCreate', message => {
  // check if it has prefix, in DMs, or not from a bot
  ...
  if (command.ownerOnly && message.author.id !== '642307176228061184') return message.reply(`${emotes.toddYell} H- hey! You're not Raven! Give me back my revolver!`);
